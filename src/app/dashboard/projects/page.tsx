  ...
  return (
    <div className="flex flex-col gap-8 max-w-6xl mx-auto w-full animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-white mb-2">Projects</h1>
          <p className="text-slate-500 dark:text-slate-400">Manage your team's projects and track progress.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 bg-white dark:bg-slate-900/50 rounded-xl border border-dashed border-slate-300 dark:border-slate-800">
          <Folder className="w-12 h-12 text-slate-400 dark:text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-slate-900 dark:text-white mb-2">No projects found</h3>
          <p className="text-slate-500 dark:text-slate-400 mb-4">Get started by creating your first project in this workspace.</p>
          <Link 
            href={activeWorkspace ? "/dashboard/projects/new" : "#"}
            className={`px-4 py-2 rounded-lg font-medium transition-colors ${
              activeWorkspace
                ? "bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-900 dark:text-white"
                : "bg-slate-100/50 dark:bg-slate-800/50 text-slate-400 dark:text-white/50 cursor-not-allowed"
            }`}
            onClick={(e) => !activeWorkspace && e.preventDefault()}
          >
            Create Project
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {projects.map((project) => (
            <Link href={`/dashboard/projects/${project.id}`} key={project.id} className="block group">
              <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 hover:border-blue-500 dark:hover:border-slate-700 transition-all cursor-pointer h-full overflow-hidden flex flex-col relative z-0">
                {project.imageUrl && (
                  <div 
                    className="h-32 w-full bg-cover bg-center border-b border-slate-200 dark:border-slate-800"
                ...
                <CardHeader className={`flex flex-row items-start justify-between pb-2 ${project.imageUrl ? 'pt-4' : ''}`}>
                  <div className="flex items-center gap-3">
                    {!project.imageUrl && (
                      <div className={`w-10 h-10 rounded-lg flex items-center justify-center bg-slate-100 dark:bg-slate-800 ${project.color} bg-opacity-20 dark:bg-opacity-10`}>
                  ...
                  <div className="relative z-10" onClick={(e) => e.preventDefault()}>
                    <DropdownMenu>
                      <DropdownMenuTrigger asChild>
                        <button className="text-slate-400 dark:text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors p-1 rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 -mr-2 -mt-2">
